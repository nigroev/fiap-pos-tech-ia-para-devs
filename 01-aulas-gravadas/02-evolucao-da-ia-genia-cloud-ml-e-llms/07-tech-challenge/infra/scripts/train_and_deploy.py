...
def deploy_sagemaker_endpoint(estimator, project, endpoint_instance_type):
    """Faz deploy do modelo treinado (GA) como endpoint de inferência."""
...
def deploy_autopilot_endpoint(auto_ml, job_name, session, project, endpoint_instance_type):
    """Faz deploy do melhor modelo do Autopilot como endpoint separado."""
    ...
    try:
        sm_client.describe_endpoint(EndpointName=endpoint_name)
        logger.info(f"Endpoint Autopilot '{endpoint_name}' já existe. Atualizando...")
        update_endpoint = True
    except sm_client.exceptions.ClientError:
        logger.info(f"Criando novo endpoint Autopilot '{endpoint_name}'...")
